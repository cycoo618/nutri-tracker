...
function DiaryHome({ onNav, onAdd, sheetOpen, closeSheet, active = '总览' }) {
  const [expanded, setExpanded] = React.useState(null); // 'macro' | 'diversity' | null
    ...
    <div className="app-paper paper-grain">
      ...
      <div className="app-scroll">
        ...
        <div style={{ padding: '14px 22px 2px' }}>
          <div style={{
            ...
            <div style={{ textAlign: 'center', minWidth: 200, position: 'relative' }}>
              <div style={{
                ...
                fontSize: 30, color: 'var(--ink)', lineHeight: 1.05,
                letterSpacing: 1,
              }}>
                {datePart}
              </div>
              <div style={{
                marginTop: 2, fontSize: 13, color: 'var(--ink-soft)',
                fontWeight: 500, letterSpacing: 2,
              }}>{weekPart} · 第 {TODAY.dayOfPlan + dayOffset} 天</div>
            </div>
          ...
          {/* Today return button — only when not on today */}
          {!isToday && (
            <div className="sheet-in" style={{ display: 'flex', justifyContent: 'center', marginTop: 8 }}>
              <button onClick={() => setDayOffset(0)} style={{
                ...
                boxShadow: '0 4px 12px color-mix(in oklab, var(--ink) 20%, transparent)',
              }}>
                <span>↩</span> 回到今天
              </button>
            </div>
          ...
          {isToday && (
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 6 }}>
              <span className="chip" style={{
                ...
                color: 'var(--moss)', fontSize: 10, padding: '2px 8px',
              }}>● 今日</span>
            </div>
          )}
        </div>
          ...
          <div className="card" style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                ...
                <span style={{ fontSize: 18, color: 'var(--ink-soft)' }}>千卡</span>
              </div>
              <div style={{ fontSize: 11, color: 'var(--ink-mute)', marginTop: 4, fontVariantNumeric: 'tabular-nums' }}>
                已记录 {TODAY.kcal} / {TODAY.kcalTarget} kcal · {Math.round(TODAY.kcal / TODAY.kcalTarget * 100)}%
              </div>
            </div>
            ...
            {/* Dashed divider */}
            <hr className="hr-dash" style={{ margin: '14px 0 12px' }} />
        ...
        <div style={{ padding: '6px 18px 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <MiniStat
            ...
            open={expanded === 'diversity'}
          />
        </div>

        {/* expanded panels */}
        {expanded === 'macro' && <MacroPanel />}
        {expanded === 'diversity' && <DiversityPanel onNav={onNav} />}
